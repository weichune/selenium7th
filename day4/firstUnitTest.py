...
# setUp和tearDown方法在每个测试用例方法执行时都会执行一次
        print(1)
    def tearDown(self):
        #tearDown()方法是在测试用例执行之后要做的操作,比如可能需要还原测试场景,或清除脏数据
        print(2)
    def test_login(self):
#       这个方法用来编写测试步骤
...
#       只有以test开头的方法才会被当做测试用例直接执行
    ...
    def switch_window(self):
        #这个方法不是test开头不能直接运行,只有被调用才会运行
        # 窗口切换方法只是希望被调用才能执行
        print(4)
    def test_zhuce(self):
        # 在python中,类里面的每个方法都有一个默认参数self
        # self类似于java中的this关键字,代表类本身
        # 如果想使用类的属性和方法,必须在前面加self关键字
        # 根据光标所在的位置,决定执行什么测试用例
        # 光标在哪个方法中,就会只运行哪个测试用例
...
#一个类中所有测试用例方法的执行顺序,是根据方法名的字母顺序决定的
    ...
    def setUpClass(cls):
        print(5)
    #@classmethod在python中叫装饰器
    #在java中叫注解
    @classmethod
    def tearDownClass(cls):
        print(6)
#   classmethod只在类中所有方法前或后执行一次

#固定写法,在程序运行时,通过这句话可以自动判断当前文件是不是程序入口
#如果当前文件是程序的入口,就会执行if子句中的内容
